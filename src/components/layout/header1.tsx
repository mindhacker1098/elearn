...
export default function Header1() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if(token){
        setUser(localStorage.getItem(`x-es-token`+token));
        console.log(localStorage.getItem(`x-es-token`+token))
      }
    }
  }, [isLoggedIn]);
      ...
      >
        ...
        >
          <HStack justifyContent={"space-around"} w={`${getMenuItems().length*10}%`}>
            <Image boxSize={14} src="/logo.png" alt="" />
          ...
          <HStack>
            {isLoggedIn ? (
                ...
                <>
                <HStack><FaUser color={"rgb(108,120,136)"} /><Text fontWeight={"600"} color={"rgb(108,120,136)"}>{user}</Text></HStack>
              <Button
                borderRadius={"0"}
